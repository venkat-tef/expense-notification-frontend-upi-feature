...
interface TokenRef { token: string; path: string }
...
async function send(tokens: TokenRef[], title: string, body: string, data: Record<string, string> = {}): Promise<void> {
...
export const onMemberCreated = onDocumentCreated('members/{uid}', async (event) => {
  ...
  await send(tokens, 'New Roommate', `${m.name} joined Nestly.`, { type: 'member_joined', url: '/settings' });
});
...
export const onAnnouncementCreated = onDocumentCreated('announcements/{id}', async (event) => {
  ...
  await send(tokens, a.title, a.body, { type: 'announcement', url: '/dashboard' });
});

// ---------- FEATURE 6: Skip Notification (Water) ----------
export const onWaterSkip = onDocumentWritten('water_records/{date}', async (event) => {
  const before = event.data?.before.data();
  const after = event.data?.after.data();
  if (!after?.skippedMemberId || before?.skippedMemberId === after.skippedMemberId) return;
  ...
  const ids = await allMemberIds();
  const tokens = await tokensForMembers(ids);
  await send(tokens, 'Water Turn Skipped', `${skipped.data()?.name ?? 'Someone'} skipped today's Water Turn. ${assigned.data()?.name ?? 'Someone'} has been assigned automatically.`, { type: 'skip', url: '/water' });
});

// ---------- FEATURE 6: Skip Notification (Garbage) ----------
export const onGarbageSkip = onDocumentWritten('cooking_records/{date}', async (event) => {
  const before = event.data?.before.data();
  const after = event.data?.after.data();
  if (!after?.skippedMemberId || before?.skippedMemberId === after.skippedMemberId) return;
  const [skipped, assigned] = await Promise.all([
  ...
  const ids = await allMemberIds();
  const tokens = await tokensForMembers(ids);
  await send(tokens, 'Garbage Turn Skipped', `${skipped.data()?.name ?? 'Someone'} skipped today's Garbage Turn. ${assigned.data()?.name ?? 'Someone'} has been assigned automatically.`, { type: 'skip', url: '/cooking' });
});

// ---------- FEATURES 2, 3, 5: Daily reminders (one scheduled run) ----------
export const dailyReminders = onSchedule({ schedule: '0 8 * * *', timeZone: TZ }, async () => {
  ...
  for (const [collectionName, label, url] of [
    ...
    const lastSnap = await db.collection(collectionName).orderBy('date', 'desc').limit(1).get();
    const lastMemberId = lastSnap.docs[0]?.data()?.memberId;
    const idx = lastMemberId ? members.findIndex((m) => m.id === lastMemberId) : -1;
    const next = members[(idx + 1 + members.length) % (members.length || 1)];
    if (!next) continue;
    const tokens = await tokensForMembers([next.id]);
    await send(tokens, `Today's ${label} Turn`, `It's your turn today.`, { type: 'duty', url });
  }

  // Features 2 & 3: Settlement / Power Bill reminders.
  const summaryDoc = await db.collection('monthly_summary').doc(monthKey).get();
  const summary = summaryDoc.data();
  if (summary && summary.settlementCompleted !== true) {
    const expenseCount = (await db.collection('expenses').where('monthKey', '==', monthKey).limit(1).get()).size;
    const ids = await allMemberIds();
    const tokens = await tokensForMembers(ids);

    if (expenseCount > 0) {
      await send(tokens, 'Reminder', 'You still have pending settlements. Open Nestly to settle them.', { type: 'settlement', url: '/expenses' });
    }
    if (summary.electricityBillSet === true) {
      await send(tokens, 'Electricity Bill Pending', "Please complete your share for this month's electricity bill.", { type: 'power_bill', url: '/expenses' });
    }
  }
});